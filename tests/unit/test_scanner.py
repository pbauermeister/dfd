...
def test_include_nonexistent_file() -> None:
    ...
    with pytest.raises(exception.DfdException, match="not found"):
        scanner.scan(provenance=None, source_text=dfd_text)
...
def test_include_snippet_not_markdown() -> None:
    ...
    with pytest.raises(exception.DfdException, match="not markdown"):
        scanner.scan(
            provenance=None, source_text=dfd_text, snippet_by_name=None
        )
...
def test_include_snippet_not_found() -> None:
    ...
    scanner.scan(
        provenance=None, source_text="#include #other", snippet_by_name=snippets
    )
    ...
    with pytest.raises(exception.DfdException, match="not found"):
        scanner.scan(
            provenance=None,
            source_text="#include #nosuchsnippet",
            snippet_by_name=snippets,
        )
...
def test_include_recursive(tmp_path: Path) -> None:
    ...
    with pytest.raises(exception.DfdException, match="Recursive"):
        scanner.scan(provenance=None, source_text=self_including.read_text())
