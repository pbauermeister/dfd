...
relevant items (in particular, `store`s) in a distinct section (file
...
`signal` and `controller` elements.
...
- Items of type `control` can only be linked by `signal`s.
...
Signals can be linked to any type of item. On the other hand,
`control`s cannot receive or emit data flows.

Eventually, signals are here to drive processes, and `control`s
