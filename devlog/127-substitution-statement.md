...
- Frames: a substitution rewrites frame membership as `~=` does today
  (phase two), nothing more.
...
4. Phase two: reads the same map; frames as today.
5. Fixtures (decision 9) and their goldens; the mutation smoke-test
...
| `src/data_flow_diagram/dsl/filters.py`                | accessor, flat map, `Substitution` case, availability, replacer branch removed                              |
