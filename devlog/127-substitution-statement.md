...
| 10  | Frames: the replacer inherits a frame only when all merged items are in that one frame; else none, and an emptied frame disappears. A replacer also declared in another frame is then in two frames: the checker's frame check, re-run after the filters, reports it ("is in multiple frames") | user (stop 0): unnamed frames cannot be chosen; the double frame is today's error; `main` put the replacer in every frame | Error on ambiguity; an option; skip inheritance for a framed replacer (the first wording of this row) |
