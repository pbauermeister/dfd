...
| 8   | Docs: filters and merge stay in chapter 7, towards the end, where filters are today; the chapter becomes "Filters and merge", and its opening (README § 7 and § 7.1 "use case", SYNTAX.md § 7 intro) says both are only useful when deriving subgraphs or simplified graphs from a master graph that carries all the details (via `#include`). README gains a merge subsection in § 7.3 and uses `merge` in § 7.4.2.2; SYNTAX.md § 7.2 loses `[=REPLACEMENT]`, a merge entry joins § 7, the glossary's "replacement" points to it. The deprecated `~=` is not mentioned | user (stop 1 discussion): placement and purpose; the docs do not mention the deprecated form                                                                                                                                                                                                                                                                                                                     | A chapter of its own for merge (would present it as a modelling statement, which it is not); a "deprecated" note                                                                                              |
...
1. README § 7: retitled "Filters and merge"; the opening and § 7.1
   state that both only serve to derive subgraphs or simplified graphs
   from a master graph. § 7.3: a "Merge" subsection (`merge ITEM... : REPLACER`,
...
2. SYNTAX.md: § 7 retitled and its intro states the same purpose;
   § 7.2 without `[=REPLACEMENT]`; a "Substitution" entry;
