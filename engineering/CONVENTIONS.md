...
- Package discovery is configured in `pyproject.toml`
  ...
  is a static `project.version`, bumped by `make release`; `make
install` stamps a copy of the tree with a local label
  (`X.Y.Z+<branch>.g<hash>[.dirty]`, `tools/print-dev-version.py`),
  the checkout keeps the bare version.
